...
（1）DEAEDDEDBCEEECECDEEDDBEEECEEDEE
   ...
   [1] https://files.catbox.moe/vz2iqg.png

（2）EEBEACCCCAADACCBCCCCECCCEEECCCE

[2]https://files.catbox.moe/2euoyu.png
---
...
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
    ...
    答：1.大写改小写，将字母的范围由0x41('A')-->0x61('a');
    
    2.大小写混用，将数组程序中的数字27全改成53，包括数组的大小，如char arr_big[27]-->char arr_big[53].

3. 使用画图工具画出上述两个Huffman生成树的生成过程

    答：[第一个] https://files.catbox.moe/y3zcat.png
    
    [第二个]https://files.catbox.moe/iuunnc.png